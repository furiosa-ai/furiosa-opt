//! E4M3 (1 sign, 4-bit exponent, 3-bit mantissa) 8-bit float conversions.
//!
//! E4M3 is not a plain bit reshuffle of f32 (its bias and saturation differ), so
//! `to_f32` uses a 256-entry lookup table and `from_f32` a hand-tuned rounding.
