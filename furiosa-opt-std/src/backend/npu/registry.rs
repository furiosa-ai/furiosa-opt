...
//!
//! What is in it: bincode of `Vec<(fn path, Key, kernel)>`, ordered by (fn path, key):
...
//! - kernel: the serialized runtime kernel ([`Bytes`]), parsed lazily per entry
//!
...
/// A registry entry's serialized kernel, parsed once by `Kernel::load`.
pub type Bytes = &'static [u8];

/// This fn's registry entries: one `(key, serialized kernel)` per launched instantiation.
pub(crate) fn entries(path: &str) -> impl Iterator<Item = (&'static Key, Bytes)> {
    static REGISTRY: OnceLock<Vec<(&str, Key, &[u8])>> = OnceLock::new();
    REGISTRY
        .get_or_init(|| match section() {
            [] => Vec::new(),
            bytes => bincode::deserialize(bytes).expect("malformed kernel registry"),
        })
        .iter()
...
fn section() -> &'static [u8] {
    ...
    unsafe { std::slice::from_raw_parts(start, stop.addr() - start.addr()) }
}
