...
fn link_furiosa_opt_lower_impl() {
    let out_dir = std::path::PathBuf::from(std::env::var("OUT_DIR").unwrap());
    let impl_target_dir = out_dir.join("furiosa-opt-lower-impl-target");

    let lib_name = "furiosa_opt_lower_impl";
    ...
    if let Ok(prebuilt) = std::env::var("FURIOSA_OPT_LOWER_IMPL_LOCAL_PREBUILT") {
        std::fs::copy(&prebuilt, &lib).expect("failed to copy FURIOSA_OPT_LOWER_IMPL_LOCAL_PREBUILT");
    } else if is_public {
        // Public (crates.io) builds fetch a prebuilt
        // libfuriosa_opt_lower_impl.a from the furiosa-ai/furiosa-opt
        // release matching this crate's version, verify SHA256SUMS, and
        // link. SHA256SUMS catches corruption but is not provenance.
        // Set FURIOSA_OPT_LOWER_IMPL_VERIFY_ATTESTATION=1 to additionally
        // check the sigstore attestation via `gh attestation verify`.

        // Pin the URL to this crate's version. Override with
        // FURIOSA_OPT_LOWER_IMPL_VERSION (e.g. "v0.1.5") for RC testing or
        ...
        if std::env::var("FURIOSA_OPT_LOWER_IMPL_VERIFY_ATTESTATION").as_deref() == Ok("1") {
    ...
    println!("cargo:rerun-if-env-changed=FURIOSA_OPT_LOWER_IMPL_VERSION");
    println!("cargo:rerun-if-env-changed=FURIOSA_OPT_GITHUB_REPO");
}

// Duplicated from `furiosa-mapping/build.rs`: a shared helper crate would have to
...
fn verify_attestation(sums: &std::path::Path) {
    let status = std::process::Command::new("gh")
        ...
        .unwrap_or_else(|e| {
            panic!(
                "FURIOSA_OPT_LOWER_IMPL_VERIFY_ATTESTATION=1 requires gh \
...
fn verify_sha256(file: &std::path::Path, sums_file: &std::path::Path, asset_name: &str) {
    ...
    let output = std::process::Command::new("sha256sum").arg(file).output().expect(
        "sha256sum is required to verify the downloaded \
             libfuriosa_opt_lower_impl archive; install coreutils and retry",
    );
    assert!(
