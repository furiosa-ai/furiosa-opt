...
The Schedule Viewer loads a schedule JSON file generated by [`cargo furiosa-opt`](./cargo-furiosa-opt.md) and displays it as an interactive execution timeline.
...
[`--dump-schedule`](./cargo-furiosa-opt.md#--dump-schedule-file) writes the compiled schedule to a JSON file.
It is available when compiling a single kernel with [`cargo furiosa-opt compile`](./cargo-furiosa-opt.md#direct-compilation-cargo-furiosa-opt-compile):
...
Provide the function name as the positional [filter](./cargo-furiosa-opt.md#filter).
