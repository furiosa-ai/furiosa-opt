...
Readers familiar with FFNs and MoE can skip to [MoE Implementation on TCP](#moe-implementation-on-tcp).
...
The TCP Vector Engine executes a fixed instruction sequence across all 256 slices simultaneously, so any operation whose address or control path depends on runtime data values must be rewritten as a fixed sequence of matrix operations:
...
- **Operations**
  ...
  - **Softmax Normalization**
    - Softmax is applied to the extracted `Top-K` Scores to calculate final weights.
      This is used in the Combine stage later.
