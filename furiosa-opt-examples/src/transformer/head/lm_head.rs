...
pub(super) fn lm_head(
    ...
    let input_it: DmTensor<bf16, Chip, Cluster, m![Y, H / 56, S / 32], m![H % 14, H / 14 % 4, S % 32]> = ctx
        ...
        .collect::<m![H % 14, H / 14 % 4, S / 16 % 2], m![S % 16]>()
        .commit_trim::<m![S % 16]>()
        .commit();
    ...
    > = ctx
        ...
        .fetch_cast::<bf16>()
        .collect::<m![S % 8, H / 56 # 64 / 16], m![H / 56 # 64 % 16]>()
    ...
    // Process vocabulary chunks (DM scratch placement is assigned automatically).
    for chunk_idx in 0..19usize {
        ...
        let weight_dm = weight_chunk.to_dm::<Cluster, m![C / 128, C / 32 % 4], m![C % 32, H]>(&mut ctx.tdma);
        ...
        let weight_it: DmTensor<bf16, Chip, Cluster, m![C / 128, H / 224], m![C % 128, H % 224]> = ctx
            ...
            .collect::<m![C / 32 % 4, C % 32, H / 16 % 14], m![H % 16]>()
            .commit_trim::<m![H % 16]>()
            .commit();
        ...
        let first: DmTensor<bf16, Chip, Cluster, m![C / 128, S / 32], m![S % 32, C % 128]> = ctx
            ...
            .contract_outer::<m![C / 4 % 32, H / 56 % 4, H / 8 % 7, S / 8 % 2, S / 32 % 4, C / 2 % 2], m![C % 2, H % 8], _, _, _>(
                &input_trf_first,
            )
            .contract_packet::<m![1]>()
