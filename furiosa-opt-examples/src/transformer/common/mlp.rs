...
pub(crate) fn mlp(
    ...
    > = ctx
        ...
        .collect::<m![S % 8, H / 8 % 7, H / 56 % 4, S / 32], m![S % 8 # 16]>()
    ...
    let gate_second: DmTensor<bf16, Chip, Cluster, m![M / 19, S / 128], m![M % 19, S % 128]> = ctx
        ...
        .collect::<m![M / 19 % 4, M % 19, H / 16 % 14], m![H % 16]>()
        .contract_outer::<m![M / 19 % 4, M % 19, H / 32 % 7], m![H % 32], _, _, _>(&input_trf_second)
        .contract_packet::<m![1]>()
        .contract_time::<m![M % 19, S / 8 % 16]>()
        .contract_lane::<m![M % 19, S / 8 % 16], m![S % 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_inter_slice_reduce::<m![M / 19, S / 128], m![M % 19, S / 8 % 16]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![S % 8 # 16]>()
        .commit_trim::<m![S % 8]>()
        .commit();
    ...
    let gate_first_raw: DmTensor<bf16, Chip, Cluster, m![M / 19, S / 128], m![M % 19, S % 128]> = ctx
        ...
        .fetch::<m![M / 19 % 4, M % 19], m![H / 16 % 14, H % 16]>()
        .collect::<m![M / 19 % 4, M % 19, H / 16 % 14], m![H % 16]>()
        .contract_outer::<m![M / 19 % 4, M % 19, H / 32 % 7], m![H % 32], _, _, _>(&input_trf_first)
        .contract_packet::<m![1]>()
        .contract_time::<m![M % 19, S / 8 % 16]>()
        .contract_lane::<m![M % 19, S / 8 % 16], m![S % 8]>(LaneMode::Interleaved)
        .vector_init()
    ...
    > = ctx
        ...
        .collect::<m![H / 112 % 2, H % 112, M % 76 # 80 / 16], m![M % 76 # 80 % 16]>()
        .commit_trim::<m![M % 76 # 80 % 16]>()
        .commit();
    ...
    let down_raw: DmTensor<bf16, Chip, Cluster, m![H / 56, S / 16], m![H % 56, S % 16]> = ctx
        ...
        .collect::<m![M % 76, H / 16 % 14], m![H % 16]>()
        .contract_outer::<m![M % 76, H / 32 % 7], m![H % 32], _, _, _>(&gated_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![1]>()
