...
pub fn fetch_table_lookup_decode_e2m1(
    ...
    let decoded: DmTensor<f32, Chip, Cluster, m![A / 16], m![A / 8 % 2, A % 8, B]> = device
        .main
        .begin(input_dm.view())
    ...
    decoded.to_hbm(&mut device.tdma)
}
...
pub fn fetch_table_lookup_decode_e2m1_to_f8e5m2(
    ...
    let input_dm = input.to_dm::<Cluster, m![A / 16], m![A / 8 % 2, A % 8, Bp]>(&mut device.tdma);

    let decoded: DmTensor<f8e5m2, Chip, Cluster, m![A / 16], m![A / 8 % 2, A % 8, Bp]> = device
        .main
        .begin(input_dm.view())
...
pub fn fetch_table_lookup_decode_e2m1_mxfp4(
    ...
    let input_dm = input.to_dm::<Cluster, m![AM / 32], m![AM / 8 % 4, AM % 8, B]>(&mut device.tdma);

    let decoded: DmTensor<f32, Chip, Cluster, m![AM / 32], m![AM / 8 % 4, AM % 8, B]> = device
        .main
        .begin(input_dm.view())
...
pub fn fetch_table_lookup_decode_f8e5m2_to_bf16(
    ...
    let input_dm = input.to_dm::<Cluster, m![A / 16], m![A / 8 % 2, A % 8, Bf]>(&mut device.tdma);

    let decoded: DmTensor<bf16, Chip, Cluster, m![A / 16], m![A / 8 % 2, A % 8, Bf]> = device
        .main
        .begin(input_dm.view())
