...
class Model(nn.Module):
    ...
    def npu(x, w1, b1, w2, b2):
        ...
        return (
            Model.EDF(x_pad, w1_pad, b1.bfloat16(), w2_pad, b2_pad)[0][:10]
            .float()
            .unsqueeze(0)
        )
...
torch.library.define(
    Model.OP, '(Tensor x, Tensor w1, Tensor b1, Tensor w2, Tensor b2) -> Tensor'
)
...
def mnist(train: bool):
    return datasets.MNIST(
        str(DIR / 'raw'), train=train, download=True, transform=TRANSFORM
    )
