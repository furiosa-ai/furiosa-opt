...
pub fn fetch_sub_slice_lift_custom_switch(
    ...
    let dm: DmTensor<bf16, Chip, Cluster, LiftAndSwitchSlice, m![H, Beat, V]> =
        input.to_dm::<Cluster, LiftAndSwitchSlice, m![H, Beat, V]>(&mut device.tdma);

    let result: DmTensor<bf16, Chip, Cluster, m![Part, H, Step], m![Beat, V]> = device
        .sub
        .begin(dm.view())
        .fetch::<m![H, Beat], m![V]>()
    ...
    result.to_hbm::<m![Part, H, Step, Beat, V]>(&mut device.tdma)
}
...
pub fn fetch_slice_lift_dynamic_view(
    ...
    let dm: DmTensor<bf16, Chip, Cluster, Slice, m![H, Win]> =
        input.to_dm::<Cluster, Slice, m![H, Win]>(&mut device.tdma);
    ...
    result.to_hbm::<m![A, H, Win]>(&mut device.tdma)
}
...
pub fn fetch_sub_slice_lift_to_vrf(
    ...
    let dm: DmTensor<i32, Chip, Cluster, Slice, m![H, Word]> =
        input.to_dm::<Cluster, Slice, m![H, Word]>(&mut device.tdma);
    let addend_dm: DmTensor<i32, Chip, Cluster, m![A, H], m![Word]> =
        addend.to_dm::<Cluster, m![A, H], m![Word]>(&mut device.tdma);

    let operand: VrfTensor<i32, Chip, Cluster, m![A, H], m![Word]> = device
        .sub
        .begin(dm.view())
        .fetch::<m![H], m![Word]>()
        .fetch_slice_lift::<m![A, H], m![1]>()
    ...
    let result: DmTensor<i32, Chip, Cluster, m![A, H], m![Word]> = device
        .main
    ...
    result.to_hbm::<m![A, H, Word]>(&mut device.tdma)
}
