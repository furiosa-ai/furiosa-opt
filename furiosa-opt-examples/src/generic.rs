...
#[device(chip = 1)]
pub fn bundle_copy<A>(ctx: &mut Context, hbm: &HbmTensor<i32, m![1], m![B, A]>) -> HbmTensor<i32, m![1], m![B, A]>
where
    A: AxisName,
{
    let dm: DmTensor<i32, m![1], m![2], m![B / 2], m![A]> = hbm.to_dm(&mut ctx.tdma);
...
pub fn pair_copy<A: AxisName, R: AxisName>(
    ...
    let dm: DmTensor<i32, m![1], m![2], m![A / 2], m![R]> = hbm.to_dm(&mut ctx.tdma);
    dm.to_hbm(&mut ctx.tdma)
}
...
mod tests {
    ...
    async fn launches() {
        let (mut ctx, f): (Context, HbmTensor<i32, m![1], m![B, F]>) = missing();
        launch(bundle_copy, (&mut ctx, &f)).await;
    }
