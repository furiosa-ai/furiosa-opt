...
async fn test_scatter_minimal() {
    ...
    let mut device = Device::new(scatter_minimal.topology()).unwrap();
    ...
    launch(scatter_minimal, (&mut device, &data, &index, &mut output))
        .await
        .unwrap();

    let actual = output.to_host::<m![C, D]>(&mut device.pdma).await.unwrap().into_vec();
